...
import { getReplay, loadReplays, deleteReplay } from '../utils/storage';
import type { ReplayFile } from '../game/types';
import { formatDuration, formatTimestamp } from '../game/replay';
import ReplayPlayer from '../components/ReplayPlayer';
import { ArrowLeft, Play, Trash2, Clock, Trophy, Disc3 } from 'lucide-react';

export default function ReplayPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [currentReplay, setCurrentReplay] = useState<ReplayFile | null>(null);
  ...
  const handlePlay = (replayId: string) => {
    navigate(`/replays/${replayId}`);
  };

  const handleDelete = (replayId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  if (id && currentReplay) {
    return (
      <div className="min-h-screen w-full bg-[#0a0f0a] relative overflow-hidden py-10 px-6">
        <div className="absolute inset-0 opacity-25 pointer-events-none">
          <div className="absolute top-[5%] left-[10%] w-32 h-32 rounded-full bg-amber-500/10 blur-3xl" />
          <div className="absolute bottom-[10%] right-[8%] w-40 h-40 rounded-full bg-emerald-500/10 blur-3xl" />
        </div>

        <div className="relative z-10 max-w-[1100px] mx-auto">
          <div className="flex items-center gap-4 mb-8">
            <div>
              <h1 className="text-3xl font-serif font-bold bg-gradient-to-b from-amber-300 to-amber-500 bg-clip-text text-transparent mb-1">
                回放播放
              </h1>
              <p className="text-zinc-500 text-sm">
                {currentReplay.players.map((p) => p.name).join(' vs ')}
              </p>
            </div>
          </div>
          <ReplayPlayer replay={currentReplay} onBack={handleBack} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-[#0a0f0a] relative overflow-hidden py-10 px-6">
      <div className="absolute inset-0 opacity-25 pointer-events-none">
        <div className="absolute top-[10%] left-[15%] w-32 h-32 rounded-full bg-amber-400/10 blur-3xl animate-pulse" />
        <div className="absolute bottom-[15%] right-[20%] w-40 h-40 rounded-full bg-emerald-500/10 blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        {replays.length === 0 ? (
          <div className="rounded-2xl bg-zinc-900/60 backdrop-blur-xl border border-zinc-700/50 p-16 text-center">
            ...
            <button
              onClick={() => navigate('/')}
              className="mt-8 px-6 py-3 rounded-xl bg-amber-600 hover:bg-amber-500 text-zinc-900 font-bold transition-all shadow-lg hover:scale-105 active:scale-95"
            >
              返回主页开始游戏
            </button>
          </div>
              ...
              >
                ...
                <div className="relative flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={(e) => { e.stopPropagation(); handlePlay(r.id); }}
                      className="p-2 rounded-lg bg-zinc-800/60 text-zinc-400 hover:bg-emerald-700/50 hover:text-emerald-200 transition-all"
                      title="播放回放"
                    >
